...
function oasis_c_nurse_report($pid, $encounter, $cols, $id) {
 $count = 0;
 $data = formFetch("forms_oasis_c_nurse", $id);
 $chart = 0;
 ...
 if ($data) {
  print "<table>\n<tr>\n";
   ...
   $key = str_replace("Oasis C Nurse", "", $key);
  ...
  if($chart == 1) { 
		...
		/* Render the form. */
		echo $c->report_action($id,'oasis_c_nurse');
	}
 }
}
?> 
