...
function oasis_nursing_resumption_report($pid, $encounter, $cols, $id) {
 $count = 0;
 $data = formFetch("forms_oasis_nursing_resumption", $id);
 $chart = 0;
...
$key = str_replace("Oasis Therapy ", "", $key);   
  ...
  if($chart == 1) { 
		...
		/* Render the form. */
		echo $c->report_action($id,'oasis_nursing_resumption');
	}
 } // End of if condition
}
