...
function oasis_discharge_report($pid, $encounter, $cols, $id) {
 $count = 0;
 $data = formFetch("forms_oasis_discharge", $id);
 $chart = 0;
...
$key = str_replace("Oasis Therapy ", "", $key);   
  ...
  print "</tr>\n</table>\n";
  if($chart == 1) { 
		$c = new C_FormPainMap('oasis_discharge','bodymap_man.png');
		/* Render the form. */
		echo $c->report_action($id,'oasis_discharge');
	}
 } // End of if condition
}
